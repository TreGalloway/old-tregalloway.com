...
var _index6 = /*#__PURE__*/require("../Has/index.js");
...
function environmentFor(has, a) {
  ...
  return M.fromEffect(T.access(r => ({
    [has.key]: (0, _index6.mergeEnvironments)(has, r, a)[has.key]
  })));
}
/**
...
function fromValue(has) {
  return resource => new _definitions.LayerManaged(M.chain_(M.fromEffect(T.succeed(resource)), a => environmentFor(has, a))).setKey(has.key);
}
/**
...
function fromEffect_(resource, has) {
  return new _definitions.LayerManaged(M.chain_(M.fromEffect(resource), a => environmentFor(has, a))).setKey(has.key);
}
/**
...
function fromManaged(has) {
  return resource => new _definitions.LayerManaged(M.chain_(resource, a => environmentFor(has, a))).setKey(has.key);
}
/**
 * Constructs a layer from a managed resource.
 */
...
function fromManaged_(resource, has) {
  return new _definitions.LayerManaged(M.chain_(resource, a => environmentFor(has, a))).setKey(has.key);
}
/**
