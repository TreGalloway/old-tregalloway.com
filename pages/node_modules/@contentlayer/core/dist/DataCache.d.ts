...
export declare namespace DataCache {
    ...
    const loadPreviousCacheFromDisk: ({ schemaHash, }: {
        schemaHash: string;
    }) => T.Effect<OT.HasTracer & HasCwd, fs.StatError | fs.ReadFileError | fs.JsonParseError | GetContentlayerVersionError, Cache | undefined>;
    const writeCacheToDisk: ({ cache, schemaHash, }: {
        cache: Cache;
        schemaHash: string;
    }) => T.Effect<OT.HasTracer & HasCwd, never, E.Either<fs.WriteFileError | fs.MkdirError | fs.JsonStringifyError | GetContentlayerVersionError, void>>;
}
