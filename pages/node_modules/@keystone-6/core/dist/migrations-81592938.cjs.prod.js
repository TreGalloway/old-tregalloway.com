...
async function pushPrismaSchemaToDatabase(dbUrl, shadowDbUrl, schema, schemaPath) {
  ...
  let migration = await withMigrate(dbUrl, schemaPath, async migrate => {
    ...
    if (migration.unexecutable.length) {
      ...
      if (!(await utils.confirmPrompt(`Do you want to continue? ${chalk__default["default"].red('All data will be lost')}.`, false))) {
    ...
    if (migration.warnings.length) {
      ...
      if (!(await utils.confirmPrompt(`Do you want to continue? ${chalk__default["default"].red('Some data will be lost')}.`, false))) {
...
async function devMigrations(dbUrl, shadowDbUrl, prismaSchema, schemaPath, shouldDropDatabase) {
  return withMigrate(dbUrl, schemaPath, async migrate => {
    ...
    if (shouldDropDatabase) {
      ...
      const devDiagnostic = await runMigrateWithDbUrl(dbUrl, shadowDbUrl, () => migrate.devDiagnostic()); // when the action is reset, the database is somehow inconsistent with the migrations so we need to reset it
      // (not just some migrations need to be applied but there's some inconsistency)

      if (devDiagnostic.action.tag === 'reset') {
        const credentials = sdk.uriToCredentials(dbUrl);
...
We need to reset the ${credentials.type} database "${credentials.database}" at ${getDbLocation(credentials)}.`);
        const confirmedReset = await utils.confirmPrompt(`Do you want to continue? ${chalk__default["default"].red('All data will be lost')}.`);
    ...
    } = await runMigrateWithDbUrl(dbUrl, shadowDbUrl, () => migrate.applyMigrations()); // Inform user about applied migrations now
    ...
    const evaluateDataLossResult = await runMigrateWithDbUrl(dbUrl, shadowDbUrl, () => migrate.evaluateDataLoss()); // if there are no steps, there was no change to the prisma schema so we don't need to create a migration
    ...
    if (evaluateDataLossResult.migrationSteps) {
      ...
      const migrationCanBeApplied = !evaluateDataLossResult.unexecutableSteps.length; // see the link below for what "unexecutable steps" are
      ...
      const migrationName = await getMigrationName(); // note this only creates the migration, it does not apply it
      ...
      const shouldApplyMigration = migrationCanBeApplied && (await utils.confirmPrompt('Would you like to apply this migration?', false));
...
async function getMigrationName() {
  let migrationName = await utils.textPrompt('Name of migration');
...
async function ensureDatabaseExists(dbUrl, schemaDir) {
  ...
  const created = await sdk.createDatabase(dbUrl, schemaDir);
