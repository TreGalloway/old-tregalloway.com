...
import { jsx, Stack, Box, useTheme, Portal } from '@keystone-ui/core';
import React, { useState, useRef, useEffect, useCallback, useContext, useMemo, memo, Fragment, createContext, forwardRef } from 'react';
import isHotkey from 'is-hotkey';
import { Editor, Transforms, Text, Path, Node, Element, Range, Point, createEditor } from 'slate';
import { ReactEditor, useSlateStatic, useFocused, useSelected, useSlate, withReact, Slate, Editable } from 'slate-react';
...
import { O as OrderableList, a as OrderableItem, D as DragHandle, R as RemoveButton, T as ToolbarGroup, b as ToolbarButton, c as ToolbarSeparator, I as InlineDialog, K as KeyboardInTooltip } from './orderable-eb864358.esm.js';
import { Button } from '@keystone-ui/button';
import { N as NotEditable, i as isValidURL } from './api-0dc0a04a.esm.js';
...
const OrderableItemInForm = /*#__PURE__*/memo(function OrderableItemInForm(props) {
  ...
  }, jsx(DragHandle, null), jsx(RemoveButton, null)), jsx(Box, {
    paddingX: "medium",
    paddingBottom: "medium"
  }, isNonChildFieldPreviewProps(props) && jsx(FormValueContentFromPreviewProps, props)));
});
...
function nestList(editor) {
  ...
  const listItemPath = Path.parent(block[1]); // we're the first item in the list therefore we can't nest
...
const LinkElement = _ref => {
  ...
  const selected = useSelected();
  const focused = useFocused();
  const [focusedInInlineDialog, setFocusedInInlineDialog] = useState(false); // we want to show the link dialog when the editor is focused and the link element is selected
...
function DocumentEditorProvider(_ref2) {
  ...
  return jsx(Slate // this fixes issues with Slate crashing when a fast refresh occcurs
  , {
    key: identity,
    editor: editor,
    value: value,
    ...
    if (!Text.isText(node) && node.type !== 'link' && node.type !== 'relationship') {
      ...
      for (const [index, childNode] of node.children.entries()) {
        ...
        if (info.kind === 'inlines') {
          ...
          if (!Editor.isBlock(editor, childNode) || // these checks are implicit in Editor.isBlock
...
export { DocumentEditor as D, ForceValidationProvider as F, assertNever as a, clientSideValidateProp as b, createDocumentEditor as c };
